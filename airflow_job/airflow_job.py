...
with DAG(
    ...
    gcs_bucket = Variable.get("gcs_bucket", default_var="airflow-projects-dev-learning")
    bq_project = Variable.get("bq_project", default_var="storied-storm-488713-b5")
    ...
    job_batch_id = f"flight-booking-batch-{env}-{str(uuid.uuid4())[:8]}"  # Full file path in GCS
    
    # Task 1: File Sensor for GCS
    file_sensor = GCSObjectExistenceSensor(
        task_id="check_file_arrival",
        bucket=gcs_bucket,
        object=f"flight-booking-analysis/scorce-{env}/flight_booking.csv",  # Full file path in GCS
    ...
    batch_details = {
        ...
        "environment_config":{
            "execution_config":{
                "service_account": "187429843413-compute@developer.gserviceaccount.com",
